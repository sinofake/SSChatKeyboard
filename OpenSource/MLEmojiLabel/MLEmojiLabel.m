...
@interface MLEmojiLabel()<SSTTTAttributedLabelDelegate>
...
//    [self initPlist];
    ...
    //点击时候的背景色
    [mutableActiveLinkAttributes setValue:(__bridge id)[[UIColor colorWithWhite:0.631 alpha:1.000] CGColor] forKey:(NSString *)kSSTTTBackgroundFillColorAttributeName];
...
- (void)attributedLabel:(SSTTTAttributedLabel *)label
...
//    
//}
-(void)setEmojiText:(NSString *)emojiText afterInheritingLabelAttributesAndConfiguringWithBlock:(NSMutableAttributedString *(^)(NSMutableAttributedString *))block{
    _emojiText = emojiText;
    if (!emojiText||emojiText.length<=0) {
        [super setText:nil];
        return;
    ...
    [self setText:mutableAttributedString afterInheritingLabelAttributesAndConfiguringWithBlock:block];
